  ...
  module Apis
    module MerchantapiNotificationsV1beta
      ...
      class MerchantService < Google::Apis::Core::BaseService
        ...
        # MCAs, the business can create a subscription for all managed accounts or for a
        # specific subaccount. We will allow the following types of notification
        # subscriptions to exist together (per business as a subscriber per event type):
        # 1. Subscription for all managed accounts + subscription for self. 2. Multiple "
        # partial" subscriptions for managed accounts + subscription for self. we will
        # not allow (per business as a subscriber per event type): 1. Multiple self
        # subscriptions. 2. Multiple "all managed accounts" subscriptions. 3. "All
        # managed accounts" subscription and partial subscriptions at the same time. 4.
